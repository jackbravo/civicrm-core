...
class CRM_Custom_Page_AJAX {
  ...
  public static function getMultiRecordFieldList() {
    ...
    if ($params['sort']) {
      ...
      // I can'f find a place where the sort is added, but it should use the name, not the label.
      $sort = CRM_Core_DAO::getFieldValue('CRM_Core_DAO_CustomField', $params['sort'][0]['column'], 'column_name', 'label');
      $obj->_DTparams['sort'] = $sort . ' ' . $params['sort'][0]['direction'];
    }
